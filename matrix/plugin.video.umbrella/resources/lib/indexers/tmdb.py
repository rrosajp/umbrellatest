...
class Auth:
	...
	def revoke_session_id(self, fromSettings=0):
		try:
			...
			url = f'{self.auth_base_link}/session?api_key={self.API_key}'
			post = {"session_id": f"{getSetting('tmdb.sessionid')}"}
			...
			if result.get('success') is True:
				...
				from resources.lib.modules import log_utils
				log_utils.log(
					f"TMDb Revoke session_id FAILED: {result.get('status_message', '')}",
					__name__,
